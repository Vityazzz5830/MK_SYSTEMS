...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const buttons = document.querySelectorAll('[class*="machinetools_"]');
    ...
    buttons.forEach(function (button) {
        button.addEventListener("click", function (event) {
            ...
            const buttonClass = [...button.classList].find(function (item) {
                return item.startsWith("machinetools_");
            });

            const id = buttonClass.replace("machinetools_", "");

            const modal = document.getElementById("machineModal_" + id);
    ...
    document.addEventListener("keydown", function (event) {
        if (event.key === "Escape") {
            const modal = document.querySelector(".machine-modal.is-open");
...
let burgerContentItems = document.querySelectorAll('.burger_content_list_item');
...
if (document.querySelector('#hamb')) {
  ...
  burgerContentItems.forEach(function(item) {
...
if (window.innerWidth < 1200) {
    document.querySelectorAll(".btn-mobfix").forEach(function(element) {
        const words = element.textContent.trim().split(/\s+/);
        element.textContent = words.slice(0, 2).join(" ");
    });
}